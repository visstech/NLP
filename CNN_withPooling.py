...
class CNN_Cancer(nn.Module):
    ...
    def __init__(self, ):
        ...
        self.pool1 = nn.MaxPool2d(kernel_size=3,stride=3) # To reduce the image size only picking useful features. Downsampled 74*74*16
        # ( width + padding -stride / stride) + 1 = 222 +0 -3 /3 => (219/3) + 1 =>  73 + 1 => 74 => 74 *74 *16
        self.conv2 = nn.Conv2d(in_channels=16,out_channels=32,kernel_size=3) # 72 * 72 *32
        self.pool2 = nn.AvgPool2d(kernel_size=2,stride=2) #() 72 +0 -2 /2 + 1) =>  35 + 1 => 36 => 36 * 36 * 32
        self.conv3 = nn.Conv2d(in_channels=32,out_channels=64,kernel_size=3) # 34 * 34 * 64 
        self.pool3 = nn.AvgPool2d(kernel_size=2,stride=2) #() 34 +0 -2 /2 + 1) =>  16 + 1 => 17 => 17 * 17 * 64
       
        #fully connected neural network
        self.fc1 = nn.Linear(17 * 17 * 64,514) # 514 hidden layers
    ...
    def forward(self,X):
        ...
        X = X.view(-1,17*17*64) #Flatten means 2d to 1d so that we can pass to neural network
